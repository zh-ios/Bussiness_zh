...
#define AH_DATE_Format @"yyyy-MM-dd"
#define AH_TIME_Format @"HH:mm:ss"

@implementation NSDate (AHDateUtil)
...
+ (NSDate *)dateWithString:(NSString *)dateString
{
    ...
    if ([dateString rangeOfString:@":"].location != NSNotFound) {
        formatter.dateFormat = [NSString stringWithFormat:@"%@ %@",AH_DATE_Format, AH_TIME_Format];
    } else {
        formatter.dateFormat = AH_DATE_Format;
    }
...
+ (NSString *)changeFromDateToString:(NSDate *)senderDate ForMat:(NSString *)formatStr
{
    ...
    return [dateFormatter stringFromDate:senderDate];
}
...
+ (NSDate *)changeFromStringToDate:(NSString *)senderString ForMat:(NSString *)formatStr
{
    ...
    return [inputFormatter dateFromString:senderString];
}


+ (NSDictionary *)theLeftFromNetTimeSinceNow:(NSString *)endDate
{
#warning TODO 读取服务器时间
//    NSTimeInterval netInterval = @"获取网络时间";
    NSTimeInterval netInterval = 1.1;
    ...
    NSDate *d=[date dateFromString:endDate];
    NSTimeZone *fromzone = [NSTimeZone systemTimeZone];
    NSInteger frominterval = [fromzone secondsFromGMTForDate:d];
    NSDate *fromDate = [d dateByAddingTimeInterval:frominterval];
    ...
    double intervalTime = [fromDate timeIntervalSinceReferenceDate] + netInterval - [localeDate timeIntervalSinceReferenceDate];
    
    NSInteger seconds		= (int) intervalTime;
    NSInteger showminutes   = 0;
    NSInteger days			= seconds / 86400 + 1;
    NSInteger hours			= seconds % 86400;
    NSInteger minutes		= hours % 3600;
    hours					= hours / 3600;
    showminutes				= minutes / 60;
    seconds					= minutes % 60;
    return [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:days],@"day",[NSNumber numberWithInteger:hours],@"hour",[NSNumber numberWithInteger:showminutes],@"minute",[NSNumber numberWithInteger:seconds],@"second", nil];
}
